...
c4.metric("設備判定", "✅ 適合" if (total_load_kva <= gen_cap*(gen_n-1)) else "❌ 容量不足")
...
fig.add_shape(type="rect", x0=0, y0=0, x1=total_box_l, y1=total_box_w, fillcolor="white", line=dict(color="black", width=1))

# データホールエリア
offset_x = corridor_w + (fwu_depth if cooling_type=="両面吹き(対面)" or cooling_type=="片面吹き" else 0)
offset_y = corridor_w
fig.add_shape(type="rect", x0=offset_x, y0=offset_y, x1=offset_x + h_l, y1=offset_y + h_w, line=dict(color="gray", dash="dash"))

# アイル・ラック列描画
curr_y = offset_y
for i in range(int(cold_aisles)):
    # ホットアイル (赤)
    fig.add_shape(type="rect", x0=offset_x, y0=curr_y, x1=offset_x+h_l, y1=curr_y+ha_w, fillcolor="rgba(255,0,0,0.1)", line=dict(color="
